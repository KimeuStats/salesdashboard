...
all_combos = targets_agg[['branch', 'category1']].drop_duplicates()
...
# === AGGREGATE ===
mtd = filtered.groupby(['branch', 'category1'])['amount'].sum().reset_index().rename(columns={'amount': 'mtd_achieved'})
daily = filtered[filtered['date'] == end_dt].groupby(['branch', 'category1'])['amount'].sum().reset_index().rename(columns={'amount': 'daily_achieved'})
prev_month = prev_year_sales[
...
pym = prev_month.groupby(['branch', 'category1'])['amount'].sum().reset_index().rename(columns={'amount': 'pym'})

df = all_combos.copy()
df = df.merge(mtd, on=['branch', 'category1'], how='left')
df = df.merge(daily, on=['branch', 'category1'], how='left')
df = df.merge(targets_agg, on=['branch', 'category1'], how='left')
df = df.merge(pym, on=['branch', 'category1'], how='left')
...
df.rename(columns={
    ...
    'achieved_vs_daily_tgt': 'Achieved vs Daily Tgt', 'mtd_tgt': 'MTD TGT', 'mtd_achieved': 'MTD Act.',
    'mtd_var': 'MTD Var', 'cm': 'CM', 'achieved_vs_monthly_tgt': 'Achieved VS Monthly tgt',
    'projected_landing': 'Projected landing', 'pym': 'PYM', 'cm_vs_pym': 'CM VS PYM'
}, inplace=True)

# === KPI DISPLAY ===
kpi1 = df['MTD Act.'].sum()
kpi2 = targets_agg['monthly_target'].sum()
...
st.markdown(f"""
<div class="kpi-grid">
    <div class="kpi-box"><h4>🏅 MTD Achieved</h4><p>{kpi1:,.0f}</p></div>
    <div class="kpi-box"><h4>🎯 Monthly Target</h4><p>{kpi2:,.0f}</p></div>
    <div class="kpi-box"><h4>📅 Daily Achieved</h4><p>{kpi3:,.0f}</p></div>
    <div class="kpi-box"><h4>📈 Projected Landing</h4><p>{kpi4:,.0f}</p></div>
    <div class="kpi-box"><h4>💼 Days Worked</h4><p>{days_worked} / {total_working_days}</p></div>
</div>
""", unsafe_allow_html=True)
